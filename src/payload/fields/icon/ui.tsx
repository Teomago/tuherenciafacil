...
// Static icon map — only includes the curated icons from icons.ts
// To add a new icon: 1) add it to icons.ts  2) import it above  3) add it here
...
export const IconRenderer: React.FC<IconRendererProps> = React.memo(({ name, size = 20 }) => {
  if (!name) return null
  const IconComponent = iconComponents[name]
  if (!IconComponent)
    return (
      <span style={{ width: size, height: size, display: 'inline-block' }} title={name}>
        ?
      </span>
    )
...
const LucideIconPicker: React.FC<TextFieldClientProps & IconPickerProps> = ({
  ...
  const drawerSlug = `icon-${path || field.name}-drawer`
