...
**`init` vs `mine`:** **`mempalace init .`** is for **first-time** room/entity detection (interactive prompts). This repo already has **`mempalace.yaml`** at the root — normal indexing is **`mempalace mine .`** only. Re-running `init` shows PEOPLE/PROJECTS choices; accept with **[enter]** or use **`mempalace init . --yes`** (and **`--no-llm`** if you want zero LLM/setup noise) when you intentionally refresh config — see `python3 -m mempalace init --help`.
