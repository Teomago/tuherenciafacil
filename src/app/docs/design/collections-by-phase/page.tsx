...
export default function CollectionsByPhase() {
  ...
  const data = [
    {
      id: 'pre', label: 'Pre-pago', title: 'Pre-pago — Registro, intake y pago',
      desc: 'El cliente se registra, llena la consulta, y paga. Al pagar se crea el caso. El abogado recibe notificación.',
      collections: [
        ...
        { name: 'Consultations', active: true, ops: ['create', 'update'], header: '#E6F1FB', headerText: '#0C447C', details: [{ who: 'client', text: 'Llena formulario de intake: datos del causante, herederos estimados, bienes conocidos. Puede guardar como borrador (status: draft) o enviar (status: submitted).' }], triggers: [] },
        { name: 'Cases', active: true, ops: ['create'], header: '#E1F5EE', headerText: '#085041', details: [{ who: 'webhook', text: 'El webhook de Wompi confirma pago → crea Case con datos copiados de la Consultation. ID: SUC-YYYY-NNN. faseActual = 0. abogadoAsignado = null.' }], triggers: ['Email al cliente: confirmación de pago', 'Email a abogados: nuevo caso en pool'] },
        { name: 'Payments', active: true, ops: ['create'], header: '#FAECE7', headerText: '#712B13', details: [{ who: 'webhook', text: 'Se crea Payment tipo "servicio" con el monto, wompiTransactionId, y fecha.' }], triggers: [] },
    ...
    },
    {
      ...
      collections: [
        ...
        { name: 'Consultations', active: false, ops: [], details: [], triggers: [] },
